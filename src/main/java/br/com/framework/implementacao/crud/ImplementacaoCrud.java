...
public class ImplementacaoCrud<T> implements InterfaceCrud<T> {
	 ...
	 * TestUnit ok
	 */
	public List<T> findListOrderByProperty(Class<T> entidade, String propriedade) {
		validaSessionFactory();
		StringBuilder query = new StringBuilder();
		query.append("select entity from ").append(entidade.getSimpleName()).append(" entity ").append(" order by entity.").append(propriedade);
		List<T> lista = sessionFactory.getCurrentSession().createQuery(query.toString()).list();
		return lista;
	}
	...
	public List<T> findListByQueryDinamica(String query, int iniciaNoRegistro,
		...
		validaSessionFactory();
		List<T> lista = new ArrayList<T>();
		lista = sessionFactory.getCurrentSession().createQuery(query.toString()).setFirstResult(iniciaNoRegistro).setMaxResults(maximoResultado).list();
		return lista;
	...
	public void executeUpdateSQLDinamica(String query, Long... cods)
			throws Exception {
		validaSessionFactory();
		sessionFactory.getCurrentSession().createSQLQuery(query).setParameterList("cods", cods).executeUpdate();
		executeFlushSession();
	}
